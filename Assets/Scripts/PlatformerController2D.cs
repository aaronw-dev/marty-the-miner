...
public class PlatformerController2D : MonoBehaviour
{
    ...
    public void platformingJump(CallbackContext ctx)
    {
        ...
        if (ctx.performed)
        {
            ...
            else if (isOnWall)
            {
                ...
                canDash = true;
                wallJumpLockedDirection = leftSideOnWall ? -1 : 1;
                wallJumpInputLockCounter = wallJumpInputLockTime;
                if (inputBuffer.x == 0)
                    spriteRenderer.flipX = !leftSideOnWall;
    ...
    public void platformingDash(CallbackContext ctx)
    {
        ...
        if (ctx.performed && canDash)
        {
            ...
            rb.linearVelocity =
                dashSpeed
                * new Vector2(Mathf.Sign(lastInputBuffer.x), isGrounded ? 1 : Mathf.Sign(lastInputBuffer.y));
    ...
    private void Update()
    {
        ...
        float speedDiff = (effectiveInputX * maxSpeed) - rb.linearVelocityX;
        float accelRate;
        if (effectiveInputX != 0)
        {
        ...
        if (!isGrounded && !isSubmerged && !isDashing && (!isOnWall || isWallJumping))
        {
            coyoteCounter -= Time.deltaTime;
            coyoteCounter = Mathf.Clamp(coyoteCounter, 0, coyoteTime);
            rb.linearVelocityY -= gravity * Time.deltaTime;
        }
        else if (isGrounded && !isSubmerged && !isJumping && !isDashing)
        {
